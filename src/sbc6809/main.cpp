...
/***************************************************************************
    ...
    Controls execution of the core sbc6809 system.
...
// ======================> sbc6809_machine_manager

class sbc6809_machine_manager : public machine_manager
{
private:
	DISABLE_COPYING(sbc6809_machine_manager);
	// construction/destruction
	sbc6809_machine_manager(emu_options &options, osd_interface &osd) : machine_manager(options, osd) { }
public:
	static sbc6809_machine_manager *instance(emu_options &options, osd_interface &osd)
	{
		if (!m_manager)
		{
			m_manager = new sbc6809_machine_manager(options, osd);
		}
		return m_manager;
	}

	static sbc6809_machine_manager *instance() { return m_manager; }

	~sbc6809_machine_manager() { delete m_manager;  m_manager = nullptr; }

	int execute()
	{
		auto system = &GAME_NAME(sbc6809);
...
private:
	static sbc6809_machine_manager* m_manager;
...
sbc6809_machine_manager* sbc6809_machine_manager::m_manager = nullptr;
...
int emulator_info::start_frontend(emu_options &options, osd_interface &osd, std::vector<std::string> &args)
{
	...
	sbc6809_machine_manager::instance(options,osd)->start_http_server();
	sbc6809_machine_manager::instance(options,osd)->execute();
	return 0;
}
...
const char * emulator_info::get_appname_lower() { return "sbc6809"; }
