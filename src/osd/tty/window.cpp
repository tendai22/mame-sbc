...
bool tty_osd_interface::window_init()
{
...
void tty_osd_interface::window_exit()
{
...
void sdl_window_info::toggle_full_screen()
{
	...
	downcast<tty_osd_interface &>(machine().osd()).release_keys();
...
void sdl_window_info::update_cursor_state()
{
	...
	if (!(machine().debug_flags & DEBUG_FLAG_OSD_ENABLED))
	{
		bool should_hide_mouse = downcast<tty_osd_interface&>(machine().osd()).should_hide_mouse();
