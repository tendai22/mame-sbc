...
tty_osd_interface::tty_osd_interface(sdl_options &options) :
...
tty_osd_interface::~tty_osd_interface()
{
}


void tty_osd_interface::init(running_machine &machine)
{
	fprintf(stderr, "tty_osd_interface::init: invoked\n");
	// call our parent
	...
	{
		osd_printf_error("tty_osd_interface: error allocating pointer data\n");
	...
	if (options().oslog())
	{
		...
		machine.add_logerror_callback(std::bind(&tty_osd_interface::output_oslog, this, _1));
	}
...
void tty_osd_interface::input_update(bool relative_reset)
{
	process_events_buf();
...
void tty_osd_interface::customize_input_type_list(std::vector<input_type_entry> &typelist)
{
...
void tty_osd_interface::release_keys()
{
...
bool tty_osd_interface::should_hide_mouse()
{
	// if we are paused, no
	if (machine().paused())
		return false;
...
void tty_osd_interface::process_events_buf()
{
	SDL_PumpEvents();
}


void tty_osd_interface::process_events()
{
...
void tty_osd_interface::osd_exit()
{
...
void tty_osd_interface::output_oslog(const char *buffer)
{
	fputs(buffer, stderr);
}


void tty_osd_interface::process_window_event(SDL_Event const &event)
{
...
void tty_osd_interface::process_textinput_event(SDL_Event const &event)
{
	if (*event.text.text)
	{
...
void tty_osd_interface::check_osd_inputs()
{
...
template <typename T>
sdl_window_info *tty_osd_interface::focus_window(T const &event) const
{
...
unsigned tty_osd_interface::map_pointer_device(SDL_TouchID device)
{
